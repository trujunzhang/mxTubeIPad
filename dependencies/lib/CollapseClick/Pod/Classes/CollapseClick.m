...
- (id)initWithFrame:(CGRect)frame {
   ...
   if (self) {
      ...
      [self reloadCollapseClick:nil withSubViews:NULL];
   }
   return self;
}
...
- (void)reloadCollapseClick:(UIImageView *)backgroundView withSubViews:(UIView *)subView {
   // Set Up: Height
   float totalHeight = 0;
   if (subView)
      totalHeight = subView.frame.origin.y + subView.frame.size.height;;
   ...
   for (int xx = 0; xx < [CollapseClickDelegate numberOfCellsForCollapseClick]; xx++) {
      // Create Cell
      CollapseClickCell * cell = [CollapseClickCell newCollapseClickCellWithTitle:[CollapseClickDelegate titleForCollapseClickAtIndex:xx]
                                                                            index:xx
                                                                          content:[CollapseClickDelegate viewForCollapseClickContentViewAtIndex:xx]];
...
- (void)closeCollapseClickCellAtIndex:(int)index animated:(BOOL)animated {
   ...
   if ([[self.isClickedArray objectAtIndex:index] boolValue] == YES) {
      ...
      [UIView animateWithDuration:duration animations:^{
          ...
          // Reposition all CollapseClickCells below Cell
          [self repositionCollapseClickCellsBelowIndex:index
                                            withOffset:-1 * (cell.ContentView.frame.size.height + kCCPad)];
      }];
   }
}
