   ...
   [request setURL:[NSURL URLWithString:
    [NSString stringWithFormat:
     @"https://www.googleapis.com/youtube/v3/channels?"
      "part=id,snippet,auditDetails,brandingSettings,contentDetails,invideoPromotion,statistics,status,topicDetails&mine=true&key=%@",
                              ...
                              if (httpresp.statusCode == 0) {
                                 ...
                                 if ([dict objectForKey:@"items"]) {
                                    ...
                                    if (items.count > 0) {
                                       ...
                                       NSString * string = _user.snippet.title;
...
- (void)getUserSubs {
   ...
   [request setURL:[NSURL URLWithString:
    [NSString stringWithFormat:
     @"https://www.googleapis.com/youtube/v3/subscriptions?part=id,snippet,contentDetails&maxResults=50&mine=true&key=%@",
     apiKey]]];
...
- (void)refreshToken {
   ...
   NSString * postbody = [NSString stringWithFormat:@"client_id=%@&client_secret=%@&refresh_token=%@&grant_type=refresh_token",
                                                    kMyClientID,
                                                    kMyClientSecret,
                                                    _token.refreshToken];
